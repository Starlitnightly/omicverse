"""
CellPLM Adapter — bridges ``omicverse.llm`` CellPLM to ``ov.fm``.
"""
...
class CellPLMAdapter(BaseAdapter):
    ...
    def __init__(self, checkpoint_dir: Optional[str] = None):
        ...
        if spec is None:
            raise RuntimeError("cellplm not found in registry")
    ...
    def run(
        ...
        model = ModelFactory.create_model("cellplm")
    ...
    def _postprocess(self, adata, embeddings, task: TaskType) -> list[str]:
        key = self.spec.output_keys.embedding_key or "X_cellplm"
