...
def fastp_clean(fq1: str, fq2: Optional[str], sample: str, work_dir: str, threads: int = 4) -> Tuple[str, Optional[str]]:
    """
    Process fq1/fq2 and write outputs to {work_dir}/fastp/{sample}_1.clean.fq.gz / _2.clean.fq.gz.
    Skip when outputs already exist and return the output paths. For single-end data, fq2/out2 are None.
    """
    outdir = Path(work_dir) / "fastp"
    outdir.mkdir(parents=True, exist_ok=True)
    out1 = outdir / f"{sample}_1.clean.fq.gz"
    out2 = outdir / f"{sample}_2.clean.fq.gz" if fq2 else None

    if out1.exists() and (out2 is None or out2.exists()):
        return str(out1), (str(out2) if out2 else None)
...
def _fastp_run_one_triplet(srr: str, fq1: str, fq2: Optional[str], work_dir: str, fastp_threads: int, retries: int):
    ...
    out2 = outdir / f"{srr}_2.clean.fq.gz" if fq2 else None
    outdir.mkdir(parents=True, exist_ok=True)

    # Skip when outputs already exist.
    if out1.exists() and (out2 is None or out2.exists()):
        return (srr, str(out1), str(out2) if out2 else None, "SKIP")
