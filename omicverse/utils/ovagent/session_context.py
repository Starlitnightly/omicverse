...
class SessionService:
    ...
    def __init__(self, ctx: Any) -> None:
        ...
        reporter = getattr(ctx, "_reporter", None)
        self._event_bus = make_event_bus(reporter)
    ...
    def restart_session(self) -> None:
        ...
        if ctx.use_notebook_execution and ctx._notebook_executor:
            if ctx._notebook_executor.current_session:
                self._event_bus.execution_status("⚙ = Manually restarting session...")
                ...
                self._event_bus.execution_status("✓ Session cleared. Next prompt will start new session.")
            else:
                self._event_bus.execution_status("💡 No active session to restart")
        else:
            self._event_bus.execution_status("⚠️  Notebook execution is not enabled")
