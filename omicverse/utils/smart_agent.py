...
from .ovagent.event_stream import EventBus as _EventBus
...
class OmicVerseAgent:
    ...
    @property
    def _event_bus(self) -> "_EventBus":
        """Lazily create EventBus for agents constructed via __new__."""
        bus = getattr(self, "_event_bus_instance", None)
        if bus is None:
            reporter = getattr(self, "_reporter", None)
            from .ovagent.event_stream import make_event_bus
            bus = make_event_bus(reporter)
            self._event_bus_instance = bus
        return bus

    @_event_bus.setter
    def _event_bus(self, value: "_EventBus") -> None:
        self._event_bus_instance = value
    ...
    def __init__(self, model: str = "gpt-5.2", api_key: Optional[str] = None, endpoint: Optional[str] = None, auth_mode: str = "environment", auth_file: Optional[str] = None, enable_reflection: bool = True, reflection_iterations: int = 1, enable_result_review: bool = True, use_notebook_execution: bool = True, max_prompts_per_session: int = 5, notebook_storage_dir: Optional[str] = None, keep_execution_notebooks: bool = True, notebook_timeout: int = 600, strict_kernel_validation: bool = True, enable_filesystem_context: bool = True, context_storage_dir: Optional[str] = None, approval_mode: str = "never", agent_mode: str = "agentic", max_agent_turns: int = 15, security_level: Optional[str] = None, *, config: Optional[AgentConfig] = None, reporter: Optional[Reporter] = None, verbose: bool = True):
        ...
        self._emit = _emit
        self._event_bus = _EventBus(self._reporter)
        ...
        backend = _resolve_model_and_provider(model, api_key, endpoint, event_bus=self._event_bus)
        ...
        try:
            ...
            self._event_bus.init(f"   📚 Function registry loaded: {stats['total_functions']} functions in {stats['categories']} categories")
            ...
            # Session history
            self._session_history = _initialize_session_history(self._config, event_bus=self._event_bus)
            ...
            self._ov_runtime = _initialize_ov_runtime(self._detect_repo_root(), event_bus=self._event_bus)
            ...
            self._event_bus.init("✅ Smart Agent initialized successfully!")
        except Exception as e:
            self._event_bus.init_error(f"❌ Agent initialization failed: {e}")
            raise
...
"""
    ...
    def _ensure_runtime_registry_for_codegen(self) -> None:
        """Ensure runtime registry is hydrated for codegen (Claw compat delegate)."""
        _RegistryScanner.ensure_runtime_registry()

    def _collect_runtime_registry_entries(
        self, request: str, max_entries: int = 8,
    ) -> List[Dict[str, Any]]:
        """Collect relevant registry entries for codegen (Claw compat delegate)."""
        return self._registry_scanner.collect_relevant_entries(request, max_entries=max_entries)
...
IMPORTANT: Respond with ONLY the JSON array, nothing else."""
    ...
    async def run_async(self, request: str, adata: Any) -> Any:
        ...
        """
        ds_shape = (adata.shape[0], adata.shape[1]) if adata is not None and hasattr(adata, 'shape') else None
        self._event_bus.request_start(request, dataset_shape=ds_shape)

        # Direct execution path for explicit Python snippets (no LLM required)
        direct_code = self._detect_direct_python_request(request)
        if direct_code:
            self._event_bus.mode_info("🧪 Direct Python detected → executing without model calls")
            self.last_usage = None
            ...
            try:
                ...
                self._event_bus.execution_status("✅ Python code executed directly.")
                return result_adata
            except Exception as exc:
                self._event_bus.execution_status(f"❌ Direct Python execution failed: {exc}")
                raise
    ...
    async def _run_agentic_mode(self, request: str, adata: Any) -> Any:
        ...
        self._event_bus.mode_info("🤖 Mode: Agentic Loop (tool-calling)")
        self._event_bus.mode_info("")

        try:
            result = await self._run_agentic_loop(request, adata)
            self._turn_controller._persist_harness_history(request)
            self._event_bus.request_success()
            return result
        except Exception as e:
            self._turn_controller._persist_harness_history(request)
            self._event_bus.request_error(str(e))
            raise
    ...
    def restart_session(self):
        ...
        if svc is not None:
            return svc.restart_session()
        # Bare-object fallback (e.g. test stubs without full __init__)
        eb = getattr(self, "_event_bus", None)
        _msg = (lambda m: eb.execution_status(m)) if eb else print
        if getattr(self, "use_notebook_execution", False) and getattr(self, "_notebook_executor", None):
            exe = self._notebook_executor
            if getattr(exe, "current_session", None):
                _msg("⚙ = Manually restarting session...")
                exe._archive_current_session()
                exe.current_session = None
                exe.session_prompt_count = 0
                _msg("✓ Session cleared. Next prompt will start new session.")
            else:
                _msg("💡 No active session to restart")
        else:
            _msg("⚠️  Notebook execution is not enabled")
