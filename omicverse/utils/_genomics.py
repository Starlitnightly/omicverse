...
def interval_dist(x, y) -> int:
    ...
    ----------
    x
        First genomic interval.
    y
        Second genomic interval.
...
def read_ctx_grn(file: os.PathLike) -> nx.DiGraph:
    ...
    ----------
    file : os.PathLike
        Input CSV file exported by ``pyscenic ctx``.

    Returns
    -------
    nx.DiGraph
        Pruned TF-target gene regulatory network.
...
def ens_trim_version(x: str) -> str:
    ...
    ----------
    x : str
        Ensembl identifier (for example ``ENSG00000141510.18``).

    Returns
    -------
    str
        Ensembl identifier without version suffix.
    """
