...
class OmicVerseLLMBackend:
    ...
    def _chat_tools_gemini(
        ...
        if self._gemini_uses_oauth_bearer(api_key):
            return self._chat_tools_gemini_rest(messages, tools, tool_choice, api_key)
    ...
    def _gemini_base_url(self) -> str:
        info = get_provider(self.config.provider)
        return str(self.config.endpoint or (info.base_url if info else "https://generativelanguage.googleapis.com/v1beta")).rstrip("/")

    def _gemini_generate_content_url(self) -> str:
        return f"{self._gemini_base_url()}/models/{quote(self._wire_model_name(), safe='')}:generateContent"

    def _gemini_cli_base_url(self) -> str:
        endpoint = str(self.config.endpoint or "").strip().rstrip("/")
        return endpoint or _GOOGLE_GEMINI_CLI_BASE_URL

    def _gemini_cli_generate_content_url(self, stream: bool = False) -> str:
        suffix = ":streamGenerateContent?alt=sse" if stream else ":generateContent"
        return f"{self._gemini_cli_base_url()}/v1internal{suffix}"
    ...
    def _clean_schema_for_gemini_cli(schema: Any) -> Any:
        if isinstance(schema, list):
            return [OmicVerseLLMBackend._clean_schema_for_gemini_cli(item) for item in schema]
        ...
        for key, value in schema.items():
            ...
            if key in {"anyOf", "oneOf"} and isinstance(value, list):
                ...
                if preferred is not None:
                    ...
                    return OmicVerseLLMBackend._clean_schema_for_gemini_cli(merged)
                continue
            cleaned_value = OmicVerseLLMBackend._clean_schema_for_gemini_cli(value)
            if key == "type" and isinstance(cleaned_value, str):
                cleaned_value = cleaned_value.upper()
            cleaned[key] = cleaned_value
        return cleaned

    def _messages_to_gemini_rest_contents(self, messages: List[Dict]) -> List[Dict[str, Any]]:
                ...
                for block in content:
                    ...
                    if block.get("type") == "text" and block.get("text"):
                        ...
                        parts.append({
                            "functionResponse": {
                                "name": block.get("name", "unknown"),
                                "response": self._gemini_function_response_payload(block.get("content", "")),
                            }
                        })
            ...
            if role == "assistant":
                ...
                parts = [{
                    "functionResponse": {
                        ...
                        "response": self._gemini_function_response_payload(message.get("content", "")),
                    }
                }]
    ...
    def _gemini_rest_generation_config(self) -> Dict[str, Any]:
        return {
            "temperature": self.config.temperature,
            "maxOutputTokens": self.config.max_tokens,
        }

    def _gemini_rest_request(self, body: Dict[str, Any], api_key: str) -> Dict[str, Any]:
        url = self._gemini_generate_content_url()
        headers = self._gemini_auth_headers(api_key)
    ...
    def _gemini_cli_request(self, body: Dict[str, Any], api_key: str) -> Dict[str, Any]:
        oauth = self._gemini_oauth_payload(api_key)
        if not oauth:
            raise RuntimeError("Gemini CLI OAuth payload is missing bearer token")
        url = self._gemini_cli_generate_content_url()
        data = json.dumps(body).encode("utf-8")
        headers = {
    ...
    def _capture_gemini_usage(self, payload: Dict[str, Any]) -> Optional[Usage]:
    ...
    @staticmethod
    def _extract_gemini_text_and_tool_calls(payload: Dict[str, Any]) -> tuple[Optional[str], List[ToolCall], Optional[Dict[str, Any]], str]:
        response_payload = payload.get("response") if isinstance(payload.get("response"), dict) else payload
    ...
    def _chat_tools_gemini_rest(
        ...
        body: Dict[str, Any] = {
            "model": self._wire_model_name(),
            "request": {
                "contents": self._messages_to_gemini_rest_contents(messages),
                "generationConfig": self._gemini_rest_generation_config(),
            },
        }
        oauth = self._gemini_oauth_payload(api_key) or {}
                ...
                "functionDeclarations": [
                    {
                        ...
                        "parameters": self._clean_schema_for_gemini_cli(tool.get("parameters", {}) or {}),
                    }
                    for tool in tools
                ]
            }]
        def _make_call() -> ChatResponse:
            payload = self._gemini_cli_request(body, api_key)
            usage = self._capture_gemini_usage(payload)
            content, tool_calls, raw_message, stop_reason = self._extract_gemini_text_and_tool_calls(payload)
            return ChatResponse(
                content=content,
                tool_calls=tool_calls,
                stop_reason=stop_reason,
    ...
    def _chat_via_gemini(self, user_prompt: str) -> str:
        ...
        if not api_key:
            raise RuntimeError("Missing GOOGLE_API_KEY for Gemini provider")
        if self._gemini_uses_oauth_bearer(api_key):
            return self._chat_via_gemini_rest(user_prompt, api_key)
        try:
            import google.generativeai as genai  # type: ignore
    ...
    def _chat_via_gemini_rest(self, user_prompt: str, api_key: str) -> str:
        body: Dict[str, Any] = {
            "model": self._wire_model_name(),
            "request": {
        ...
        def _make_call() -> str:
            payload = self._gemini_cli_request(body, api_key)
            self._capture_gemini_usage(payload)
            content, _tool_calls, _raw_message, _stop_reason = self._extract_gemini_text_and_tool_calls(payload)
            return content or ""
    ...
    async def _stream_gemini(self, user_prompt: str):
        ...
        if not api_key:
            raise RuntimeError("Missing GOOGLE_API_KEY for Gemini provider")
        if self._gemini_uses_oauth_bearer(api_key):
            result = await asyncio.to_thread(self._chat_via_gemini_rest, user_prompt, api_key)
            if result:
                yield result
            return
