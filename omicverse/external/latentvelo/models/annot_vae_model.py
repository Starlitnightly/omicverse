...
from .modules import create_encoder, create_decoder, MLP, GCN
from ..utils import normalize, sparse_mx_to_torch_sparse_tensor, batch_jacobian, gaussian_kl, paired_correlation, unique_index, as_float_ndarray
...
def set_device(d):
    ...
    print(f'LatentVelo AnnotVAE using device: {device}')
    
class AnnotVAE(nn.Module):
    """
    LatentVelo Celltype Annotated VAE model.
    ...
    """
    def __init__(self, observed, celltypes, latent_dim = 20, zr_dim = 2, h_dim = 2,
        ...
        # encoder networks
        self.encoder_z0_s, self.encoder_z0_u, self.encoder_z_s, self.encoder_z_u = create_encoder(observed, latent_dim, encoder_hidden, batch_correction, batches, shared, encoder_bn)
        
        if self.exp_time:
            self.encoder_u = nn.Sequential(
                nn.Linear(2*self.latent + celltypes+1, 4*self.latent),
                nn.ELU(),
                nn.Linear(4*self.latent, 2*self.u_size))
        else:
            self.encoder_u = nn.Sequential(
                nn.Linear(2*self.latent + celltypes, 4*self.latent),
                nn.ELU(),
                nn.Linear(4*self.latent, 2*self.u_size))
        
        # maybe add celltype to c encoder and t encoder...
        if self.gcn:
            self.encoder_c = GCN(self.latent, 2*self.h_dim, gcn_layers=1, combine=True)
            self.encoder_t = GCN(self.latent, 2, gcn_layers=1, combine=True)
        else:
            self.encoder_c = MLP(2*self.latent, [2*self.latent], 2*self.h_dim)
            self.encoder_t = MLP(2*self.latent, [2*self.latent], 2)
        
        if self.exp_time:
            self.z_decoder = nn.Sequential(nn.Linear(self.u_size+celltypes+1, 4*self.latent), nn.ELU(), nn.Linear(4*self.latent, 4*self.latent))
        else:
            self.z_decoder = nn.Sequential(nn.Linear(self.u_size+celltypes, 4*self.latent), nn.ELU(), nn.Linear(4*self.latent, 4*self.latent))
        
        # decoder network
        self.decoder_s, self.decoder_u = create_decoder(observed, latent_dim, decoder_hidden, linear_decoder, batch_correction, batches, shared, decoder_bn)
        
        # velocity field network
        self.velocity_field = VelocityFieldReg(self.latent, self.h_dim, self.zr_dim, include_time)
        ...
        # initial state
        self.initial = nn.Linear(1, 2*self.latent) 
    ...
    def loss(self, normed_s, s, s_size_factor, mask_s, normed_u, u, u_size_factor, mask_u, velo_genes_mask, adj, root_cells, obs_celltype=(None, None, None), batch_id = (None, None), epoch = None):
        ...
        latent_state, latent_mean, latent_logvar, latent_time, time_mean, time_logvar = self.latent_embedding(normed_s, normed_u, obs_celltype, adj, batch_id = batch_onehot, exp_time = exp_time)
        ...
        ulatent = ulatent[sort_index][index]
        obs_celltype = obs_celltype[sort_index][index]
        celltype_id = celltype_id[sort_index][index]
        if self.exp_time:
        ...
        if self.likelihood_model == 'gaussian':
            ...
            validation_velo = validation_traj
        ...
        if epoch != None:
            ...
            if self.exp_time:
                z_decoder_params = self.z_decoder(th.cat((ulatent, obs_celltype, exp_time), dim=-1))
            else:
                z_decoder_params = self.z_decoder(th.cat((ulatent, obs_celltype), dim=-1))
            z_decoder_prob = Normal(z_decoder_params[:,:2*self.latent], th.exp(0.5*z_decoder_params[:,2*self.latent:]))
            z_posterior = Normal(latent_mean[:,:2*self.latent], th.exp(0.5*latent_logvar[:,:2*self.latent]))
            
            kl_reg = self.kl_final_weight * min(1, epoch/self.kl_warmup_steps) * (kl_divergence(z_posterior, z_decoder_prob).sum(-1) + gaussian_kl(latent_mean[:,self.latent*2:self.latent*2+self.u_size], latent_logvar[:,self.latent*2:self.latent*2+self.u_size]) + 0.1*gaussian_kl(time_mean[:,None], time_logvar[:,None]))
        else:
            if self.exp_time:
                z_decoder_params = self.z_decoder(th.cat((ulatent, obs_celltype, exp_time), dim=-1))
            else:
            ...
            kl_reg = self.kl_final_weight* (kl_divergence(z_posterior, z_decoder_prob).sum(-1) + gaussian_kl(latent_mean[:,self.latent*2:self.latent*2+self.u_size], latent_logvar[:,self.latent*2:self.latent*2+self.u_size]) + 0.1*gaussian_kl(time_mean[:,None], time_logvar[:,None]))
    ...
    def latent_embedding(self, normed_s, normed_u, obs_celltype, adj, batch_id = None, exp_time = None):
        ...
        if self.exp_time:
            u_params = self.encoder_u(th.cat((z, obs_celltype, exp_time), dim=-1))
        else:
            u_params = self.encoder_u(th.cat((z, obs_celltype), dim=-1))
        u_mean, u_logvar = u_params[:,:self.u_size], u_params[:,self.u_size:]
        ulatent = u_mean + th.randn_like(u_mean)*th.exp(0.5*u_logvar)
        
        return th.cat((z, ulatent, context), dim=-1), th.cat((zs_mean, zu_mean, u_mean, context_mean), dim=-1), th.cat((zs_logvar, zu_logvar, u_logvar, context_logvar), dim=-1), latent_time, t_mean, t_logvar
    ...
    def reconstruct_latent(self, normed_s, normed_u, obs_celltype, adj, batch_id = None):
        ...
        latent_state, latent_mean, latent_logvar, latent_time, time_mean, time_logvar = self.latent_embedding(normed_s, normed_u, obs_celltype, adj, batch_id = batch_id, exp_time = exp_time)
        ...
        return th.cat((z,zt), dim=-1), th.cat((zs,zu,zt), dim=-1), velocity, latent_time, c, ulatent
    ...
    def latent_reg_func(self, zs, zu, zr, zs_data, zu_data, latent_time):
        ...
        #bio_reg = 10000*( F.relu(s_degradation).sum(dim=(-1,-2)) + F.relu(-1*s_splicing).sum(dim=(-1,-2)) + F.relu(u_splicing).sum(dim=(-1,-2)) + F.relu(-1*t_production).sum(dim=(-1,-2)) + F.relu(t_degradation).sum(dim=(-1,-2)))
        #return 10000*( F.relu(-1*s_splicing).sum(dim=(-1,-2)) + F.relu(u_splicing).sum(dim=(-1,-2)) )
        #return 10000*( F.relu(s_degradation).sum(dim=(-1,-2)) + F.relu(-1*s_splicing_data).sum(dim=(-1,-2)) + F.relu(-1*s_splicing).sum(dim=(-1,-2)) + F.relu(u_splicing).sum(dim=(-1,-2)) )
        #return 10000*( F.relu(s_degradation).sum(dim=(-1,-2)) + F.relu(-1*s_splicing).sum(dim=(-1,-2))) #+ F.relu(u_splicing).sum(dim=(-1,-2)) )
    ...
    def velo_reg_func(self, normed_s, normed_u, shat, uhat, shat_data, uhat_data, mask_s, mask_u, zs, zu, zt, zs_data, zu_data, latent_time, batch_id, celltype_id, velo_genes_mask):
        ...
        if self.celltype_velo:
            ...
            for i in range(self.celltypes):
                ...
                loss.append( th.sum((splicing_velo_data - gene_velocity_data[celltype_id == i])[:,velo_genes_mask==1]**2, dim=-1) + th.sum((splicing_velo - gene_velocity[celltype_id == i])[:,velo_genes_mask==1]**2, dim=-1)  )
            loss = th.cat(loss, dim=0) 
        else:
