...
parser.add_argument('--tgthighres', type=int, default=4, help='high res')
    ...
    for i in tqdm(range(gexpr_feature.shape[0])):
        with torch.no_grad():
            if pretrainconfig['rawcount'] == False:
                # tmpdata = (gexpr_feature.iloc[i,:]/(gexpr_feature.iloc[i,:].sum())*1e4).tolist()
                ...
                tmpdata = gexpr_feature.iloc[i,:].tolist()
                pretrain_gene_x = torch.tensor(tmpdata+[args.tgthighres,totalcount]).unsqueeze(0).cuda()
