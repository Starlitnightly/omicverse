...
class TransformerModel(nn.Module):
    def __init__(
        ...
        if topo_graph:
            ...
            self.layer_emb=TopoLayerEncoding(d_model) if layer_emb else None
    ...
    def generate(
        ...
        pretrain_mask=generate_square_subsequent_mask(src.size(-1),device=src.device)>0
        try:
            self._check_batch_labels(batch_labels)
        except:
            import warnings
        ...
        transformer_output = self.transformer_encoder(
            total_embs, mask=pretrain_mask,src_key_padding_mask=src_key_padding_mask,is_causal=True
        )
        ...
        output ={}
        output["causal_output"]= mlm_output["pred"]# (batch, seq_len)
    ...
    def forward(
        ...
        if self.topo_graph:
            output["lm_logit"]=self.topo_forward(input_sorted_gene=input_sorted_gene,topo_padding_mask=topo_padding_mask,
                                                 sorted_layer_idx=sorted_layer_idx)
        return output

    def topo_forward(self,input_sorted_gene,topo_padding_mask,sorted_layer_idx=None,target_sorted_gene=None):
        '''
        ...
        Returns:
        '''
        emb=self.encoder(input_sorted_gene)#[bsz,seq_len,emb_size], with mask
        if self.layer_emb is not None:
            emb=self.layer_emb(emb,sorted_layer_idx)
        h=self.transformer_encoder(emb, src_key_padding_mask=topo_padding_mask)
...
def generate_square_subsequent_mask(sz: int,device) -> Tensor:
