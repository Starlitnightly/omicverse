...
def tokenize_batch(
    ...
    for i in range(len(data)):
        ...
        if graph is not None:
            ...
            sort_layer_idx = np.array(sort_layer_idx)
            genes=grn.ndata['_ID'][sorted_index]
            values=values[sorted_index]
            sorted_index = sorted_index.detach().cpu().numpy()
        else:
            sort_layer_idx=torch.zeros_like(torch.from_numpy(gene_ids)) if return_pt else np.zeros_like(gene_ids)
        genes = genes if isinstance(genes, np.ndarray) else genes.detach().cpu().numpy()
        values = values if isinstance(values, np.ndarray) else values.detach().cpu().numpy()
        sort_layer_idx = sort_layer_idx if isinstance(sort_layer_idx, np.ndarray) else sort_layer_idx.detach().cpu().numpy()
        if return_pt:
            genes = torch.from_numpy(genes).long()
            values = torch.from_numpy(values)
            sort_layer_idx = torch.from_numpy(sort_layer_idx).long()
            sorted_index = torch.from_numpy(sorted_index).long()
        tokenized_data.append((genes, values,sort_layer_idx, sorted_index))
...
def pad_batch(
    ...
    for i in range(len(batch)):
        gene_ids, values ,sorted_layer_idx, sorted_index = batch[i]
        if len(gene_ids) > max_len:
            ...
            if graph is not None:
                ...
                sorted_layer_idx=torch.zeros_like(gene_ids) if isinstance(gene_ids, torch.Tensor) else np.zeros_like(gene_ids)
        if len(gene_ids) < max_len:
            if isinstance(gene_ids, torch.Tensor):
                ...
                gene_ids = np.concatenate([gene_ids, np.full((max_len - len(gene_ids),), pad_id, dtype=gene_ids.dtype)])
                values = np.concatenate([values, np.full((max_len - len(values),), pad_id, dtype=values.dtype)])
            if graph is not None:
                if isinstance(sorted_layer_idx, torch.Tensor):
                    ...
                    sorted_index = torch.cat(
                        [
                            sorted_index,
                            torch.full((max_len - len(sorted_index),), pad_value, dtype=sorted_index.dtype),
                        ]
                    )
                else:
                    sorted_layer_idx = np.concatenate([sorted_layer_idx, np.full((max_len - len(sorted_layer_idx),), pad_id, dtype=sorted_layer_idx.dtype)])
                    sorted_index = np.concatenate([sorted_index, np.full((max_len - len(sorted_index),), pad_id, dtype=sorted_index.dtype)])
            else:
                sorted_layer_idx = torch.zeros_like(gene_ids) if isinstance(gene_ids, torch.Tensor) else np.zeros_like(gene_ids)
    ...
    if isinstance(gene_ids, torch.Tensor):
        batch_padded = {
            ...
            "sorted_layer_idx":torch.stack(sorted_layer_idx_list, dim=0) if graph else None,

        }
        if graph is not None:
            batch_padded['sorted_index'] = torch.stack(sorted_index_list, dim=0)
    else:
        batch_padded = {
            "genes": np.stack(gene_ids_list, axis=0),
            "values": np.stack(values_list, axis=0),
            "sorted_layer_idx": np.stack(sorted_layer_idx_list, axis=0) if graph else None,

        }
        if graph is not None:
            batch_padded['sorted_index'] = np.stack(sorted_index_list, axis=0)
    return batch_padded
...
def tokenize_and_pad_batch(
    ...
    batch_padded = pad_batch(
        tokenized_data, max_len, vocab, pad_token, pad_value, cls_appended=append_cls,graph=graph
    )
    # else:
    #
    #     gene_ids_list,values_list=[gene for gene,value in tokenized_data],[value for gene,value in tokenized_data]
