...
class Preprocessor:
    ...
    def __init__(
        ...
        filter_gene_by_counts: Union[int, bool] = 3,
        filter_cell_by_counts: Union[int, bool] = 200,
        normalize_total: Union[float, bool] = None,
        ...
        r"""
        Set up the preprocessor, use the args to configs the workflow steps.
    ...
    def __call__(self, adata: AnnData, batch_key: Optional[str] = None) -> Dict:
        ...
        if self.normalize_total is None and self.log1p is None:
            if adata.X.max() > 20:
                self.log1p = True
            else:
                self.log1p = False
        ...
        if (
            ...
            sc.pp.filter_cells(
                adata,
                min_genes=self.filter_cell_by_counts
        ...
        # step 5: subset hvg
        if self.subset_hvg and adata.shape[1] > self.subset_hvg:
        ...
        # step 6: binning
        if self.model_used in ["scgpt", "scmamba"]:
            if self.binning:
                print("Binning data ...")
...
def preprocess_adata(adata, n_hvg=False):
    normalize_total = 1e4
    log1p = True
    if adata.X.max() < 20:
        log1p = False
