...
def ProcessInputData(train_data, test_data, sep=None, datatype='counts', variance_threshold=0.98,
                     ...
                     genelenfile=None):
    ### read train data
    print('Reading training data {}'.format(datatype))
    ...
    print("train shape is {}".format(train_x.shape))
    ...
    if scaler=='ss':
       ...
       # print(train_x.isna().sum())
        ...
        print("Using none scaler...")

        mms_train_x = np.array(train_x)
        mms_test_x =  np.array(test_x)
        #print(mms_train_x,mms_test_x)
        sns.histplot(data=np.mean(mms_train_x, axis=0), kde=True, color=colors[3],edgecolor=None)
        sns.histplot(data=np.mean(mms_test_x, axis=0), kde=True, color=colors[7],edgecolor=None)
        plt.legend(title='datatype', labels=['trainingdata', 'testdata'])
