...
class FCLayers(nn.Module):
    ...
    def __init__(
        ...
        self.use_context_network = use_context_network
        if use_context_network:
            layers_dim = [n_in] + (n_layers - 1) * [context_n_hidden] + [n_out]
            self.fc_layers_context = nn.Sequential(
                collections.OrderedDict(
                    [
                        (
                            "Context_Layer_{}".format(i),
                            nn.Sequential(
                                nn.Dropout(p=dropout_rate) if dropout_rate > 0 else None,
                                nn.Linear(
                                    n_in + cat_dim * self.inject_into_layer(i),
    ...
    def forward(self, x: torch.Tensor, *cat_list: int):
        ...
        if self.use_context_network:
            x_context = x
            for i, layers in enumerate(self.fc_layers_context):
                for layer in layers:
                    if layer is not None:
                        if isinstance(layer, nn.BatchNorm1d):
                            if x_context.dim() == 3:
                                x_context = torch.cat([(layer(slice_x)).unsqueeze(0) for slice_x in x_context], dim=0)
                            else:
                            ...
                            if isinstance(layer, nn.Linear) and self.inject_into_layer(i):
                                if x_context.dim() == 3:
                                    one_hot_cat_list_layer = [
                                        o.unsqueeze(0).expand((x_context.size(0), o.size(0), o.size(1)))
                                        for o in one_hot_cat_list
                                    ]
                                else:
                                    one_hot_cat_list_layer = one_hot_cat_list
                                x_context = torch.cat((x_context, *one_hot_cat_list_layer), dim=-1)
                            x_context = layer(x_context)
        for i, layers in enumerate(self.fc_layers):
            for layer in layers:
                if layer is not None:
                    if isinstance(layer, nn.BatchNorm1d):
                        ...
                        x = layer(x)
        if self.use_context_network:
            x = x * torch.nn.functional.softmax(x_context, dim=1)
        return x
